...
export const useSocketStore = defineStore('socket', () => {
    ...
    try {
      ...
      if (newTiles.length !== playerTileLetters.length) {
        logger.warn(
          ...
          'Tile System',
          {
            expected: playerTileLetters.length,
            actual: newTiles.length,
  ...
  function connect(): Promise<void> {
      ...
      (data: { status_code: number; message: string; payload: string }) => {
        ...
        if (data.status_code === 200 && data.payload) {
          try {
            ...
            if (currentPlayerName && gameData.players && gameData.players[currentPlayerName]) {
              const playerTileLetters: string[] = gameData.players[currentPlayerName]
    ...
    socket.value.on('game_state', (data: GameState) => {
      ...
      // Extract current player's tiles and convert to Tile objects
      const currentPlayerName = playerStore.playerName
      if (currentPlayerName && data.players && data.players[currentPlayerName]) {
  ...
  function joinGame(gameId: string, playerName: string, testMode: boolean = false) {
    if (!socket.value?.connected) {
