...
export function useDragDrop() {
  ...
  function handleCellDrop(targetRow: number, targetCol: number) {
    ...
    if (!validateTileOperation(dragData.tileId, 'Cell Drop')) {
      return
    }
    ...
    const tilesToMove =
      playerStore.selectedTiles.size > 0 && playerStore.isSelected(dragData.tileId)
        ? (Array.from(playerStore.selectedTiles)
    ...
    for (const tile of tilesToMove) {
      ...
      if (tilePos) {
        ...
        if (
          newRow < 0 ||
          newRow >= boardStore.boardSize ||
          newCol < 0 ||
          newCol >= boardStore.boardSize
        ) {
      ...
      try {
        if (move.fromRow !== undefined && move.fromCol !== undefined) {
          ...
          if (boardStore.getTileAt(move.toRow, move.toCol)) {
            logger.error(`Target cell (${move.toRow}, ${move.toCol}) is occupied`, 'Drag Drop', {
              targetRow: move.toRow,
              targetCol: move.toCol,
            })
            continue
          }
        ...
        logger.error(`Error executing move for tile ${move.tile.id}`, 'Drag Drop', {
          error,
          tile: move.tile,
          move,
        })
  ...
  function handleBenchDrop() {
    const dragData = uiStore.dragData
    if (!dragData || dragData.sourceBench) {
      return
    }
